...
| Сервер API | `https://cryptuna-anderm.amvera.io/v1/` |
...
- Деплой через Amvera (nginx, конфиг в `.nginx/`)
...
> ⚠️ Страницы `catalog` и `wallet` из старой версии отсутствуют в новой.
...
### CustomFetch
- Базовый URL: `https://cryptuna-anderm.amvera.io/v1/`
...
- **Перерендер UI после отправки отзыва** — прыгают компоненты
...
- **Фото/изображения в шагах** — поддержка изображений внутри контента шага не реализована
- **Светлая тема** — только тёмная тема, светлая не реализована
- **Страница Wallet** — отсутствует полностью
- **Страница Catalog** — отсутствует

#### Telegram-интеграция (для деплоя)
- **Кнопка "Назад" Telegram** — `tg.BackButton` не подключена
- **Свайпы** — `tg.disableVerticalSwipes()` / обработка свайпов
